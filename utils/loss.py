...
class HybridLoss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        assert (pred>=0).all() and (pred<=1).all(), 'pred value range error'
        assert (target>=0).all() and (target<=1).all(), "target value range error"
        smooth = 1
        target = target.float()
        ...
        bceloss = self.bce_loss(pred,target)
        assert diceloss>=0, 'DiceLoss<0'
        assert bceloss>=0,'BCELoss<0'
        return diceloss+self.bce_weight*bceloss
